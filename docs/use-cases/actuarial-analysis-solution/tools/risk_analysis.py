...
class RiskAnalysisService:
    ...
    def analyze_risk_factors(self, claims_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            ranked_factors = sorted(factor_analyses, key=lambda x: x['significance_score'], reverse=True)
            ...
            return {
                ...
                'summary': {
                    ...
                    'significant_factors': sum(1 for f in factor_analyses if f['is_significant']),
                    'total_claims': len(df)
                }
            }
    ...
    def _identify_risk_factors(self, df: pd.DataFrame) -> List[str]:
        ...
        categorical_columns = ['lineofbusiness', 'claimstatus', 'losstype', 'causeofloss', 'garagestate', 'accidentstate']
        ...
        if 'paidtotal' in df.columns:
            df['amount_category'] = pd.cut(df['paidtotal'], 
    ...
    def _analyze_single_factor(self, df: pd.DataFrame, factor: str) -> RiskFactor:
        ...
        for segment in segments:
            ...
            avg_paid = segment_df['paidtotal'].mean() if 'paidtotal' in segment_df.columns else 0
    ...
    def _calculate_statistical_significance(self, segment_data: List[np.ndarray]) -> float:
        try:
    ...
    def _generate_risk_insights(self, df: pd.DataFrame, factor_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        insights = {}
        ...
        insights['high_risk_segments'] = sorted(high_risk_segments, 
                                              key=lambda x: x['loss_ratio'], 
    ...
    def _identify_emerging_patterns(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        if 'accident_date' in df.columns:
            ...
            if len(monthly_claims) > 3:
                ...
                historical_avg = monthly_claims.head(-3).mean() if len(monthly_claims) > 6 else monthly_claims.mean()
    ...
    def detect_risk_trends(self, historical_data: List[Dict[str, Any]], current_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        try:
            ...
            if len(historical_df) > 0 and len(current_df) > 0:
                ...
                freq_change = ((current_freq - historical_freq) / historical_freq * 100) if historical_freq > 0 else 0
                
                trends['frequency_trend'] = {
                    'historical_count': historical_freq,
                    'current_count': current_freq,
                    'change_percent': freq_change,
                    'trend_direction': 'increasing' if freq_change > 5 else 'decreasing' if freq_change < -5 else 'stable'
                }
                
                # Severity trend
                if 'paidtotal' in historical_df.columns and 'paidtotal' in current_df.columns:
                    historical_avg = historical_df['paidtotal'].mean()
                    current_avg = current_df['paidtotal'].mean()
                    severity_change = ((current_avg - historical_avg) / historical_avg * 100) if historical_avg > 0 else 0
                    
                    trends['severity_trend'] = {
                        'historical_average': historical_avg,
                        'current_average': current_avg,
                        'change_percent': severity_change,
                        'trend_direction': 'increasing' if severity_change > 10 else 'decreasing' if severity_change < -10 else 'stable'
                    }
            
            # Risk factor stability analysis
            risk_factor_stability = self._analyze_risk_factor_stability(historical_df, current_df)
    ...
    def _analyze_risk_factor_stability(self, historical_df: pd.DataFrame, current_df: pd.DataFrame) -> Dict[str, Any]:
        stability_analysis = {}
        
        # Analyze line of business distribution if available
        if 'line_of_business' in historical_df.columns and 'line_of_business' in current_df.columns:
    ...
    def _generate_trend_recommendations(self, trends: Dict[str, Any]) -> List[str]:
        ...
        if 'frequency_trend' in trends:
            ...
            if freq_trend['trend_direction'] == 'increasing':
                recommendations.append("Monitor increasing claim frequency - consider underwriting review")
            elif freq_trend['trend_direction'] == 'decreasing':
                recommendations.append("Decreasing claim frequency is positive - maintain current practices")
        
        # Severity recommendations
        if 'severity_trend' in trends:
            sev_trend = trends['severity_trend']
            if sev_trend['trend_direction'] == 'increasing':
                recommendations.append("Rising claim severity detected - review reserve adequacy")
            elif sev_trend['trend_direction'] == 'decreasing':
                recommendations.append("Decreasing claim severity - potential reserve release opportunity")
