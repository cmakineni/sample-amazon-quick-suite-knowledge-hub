...
def lambda_handler(event, context):
    try:
        logger.info(f"AgentCore Lambda invoked with tool: {event.get('tool_name', 'unknown')}")
        
        memory_id = os.environ.get('AGENTCORE_MEMORY_ID')
        if not memory_id:
            return {
                'statusCode': 500,
                'body': json.dumps({'error': 'AGENTCORE_MEMORY_ID environment variable not set'})
            }
        
        tool_name = context.client_context.custom.get('bedrockAgentCoreToolName', '') if context.client_context and context.client_context.custom else ''
        if '___' in tool_name:
        ...
        if context.client_context and context.client_context.custom:
            context_actor_id = context.client_context.custom.get('actorId') or context.client_context.custom.get('actor_id')
            if context_actor_id:
        ...
        try:
            ...
            if df is not None and not df.empty:
                logger.info(f"Session data loaded successfully: {len(df)} records, {len(df.columns)} columns")
                try:
                    data_event = df.to_dict('records')
                except Exception as convert_error:
                    logger.error(f"Failed to convert DataFrame to records: {str(convert_error)}")
                    return {
                        'statusCode': 500,
                        'body': json.dumps({'error': f'DataFrame conversion failed: {str(convert_error)}'})
                    }
            else:
                return {
                    'statusCode': 404,
                    'body': json.dumps({'error': f'No data found for session_id: {session_id}'})
                }
                
        except Exception as load_error:
            return {
                'statusCode': 500,
                'body': json.dumps({'error': f'Failed to load session data: {str(load_error)}'})
            }
        ...
        if tool_name == 'detect_litigation':
            ...
            result = litigation_analysis.detect_litigation(data_event, litigation_config)
            ...
            logger.info(f"Triangle construction result keys: {list(result.keys()) if isinstance(result, dict) else 'Not a dict'}")
            ...
            try:
                ...
                for event_item in events:
                    try:
                        payload_blob = event_item.get('payload', [{}])[0].get('blob', '{}')
                        event_data = json.loads(payload_blob)
                        
                        if event_data.get('event_type') == 'triangle_result':
                            triangles_data = event_data
                            print(f"=== FOUND TRIANGLE DATA IN MEMORY ===")
                            if 'incurred_triangle' in triangles_data and 'data' in triangles_data['incurred_triangle']:
                                sample_data = triangles_data['incurred_triangle']['data']
                                if sample_data:
                                    first_key = list(sample_data.keys())[0]
                                    columns = list(sample_data[first_key].keys()) if sample_data[first_key] else []
                ...
                if not triangles_data:
                    ...
                    triangle_data_to_store = {
                        'event_type': 'triangle_result',
                        'session_id': session_id,
                        'incurred_triangle': triangle_result.get('incurred_triangle', {}),
                    ...
                    try:
                        ...
                        print(f"Stored new triangle data in AgentCore memory for session: {session_id}")
            ...
            return {
                ...
                'body': json.dumps({
                    ...
                    'available_tools': [
                        'detect_litigation', 'score_fraud_risk',
