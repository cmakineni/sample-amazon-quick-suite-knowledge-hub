...
from .constants import FIELD_MAPPINGS, AWS_CONFIG
    ...
    try:
        ...
        if not s3_path:
            ...
            if dataframe_records:
                ...
                raise ValueError(f"No s3_parquet_path or dataframe found for session_id: {session_id}")
        ...
        for i, event_item in enumerate(events):
            try:
                ...
                if hasattr(event_item, 'messages') and event_item.messages:
                    message_content = event_item.messages[0][0] if isinstance(event_item.messages[0], tuple) else event_item.messages[0]
                    event_content = json.loads(message_content) if isinstance(message_content, str) else message_content
                elif isinstance(event_item, dict) and 'payload' in event_item:
                    # Try payload structure
                    payload = event_item['payload']
                    if isinstance(payload, list) and len(payload) > 0:
                        blob_data = payload[0].get('blob') if isinstance(payload[0], dict) else payload[0]
                        event_content = json.loads(blob_data) if isinstance(blob_data, str) else blob_data
                    else:
                ...
                print(f"Event {i} content keys: {list(event_content.keys()) if isinstance(event_content, dict) else 'Not a dict'}")
                ...
                if event_content and isinstance(event_content, dict):
                    ...
                    if event_type == 'query_result':
                        print(f"Found query result event with keys: {list(event_content.keys())}")
...
def store_session_metadata(session_id: str, s3_parquet_path: str, row_count: int, columns: list, query: str = "") -> bool:
    """
    ...
    try:
        ...
        agentcore.create_event(
            ...
            payload=[
                {
                    "blob": json.dumps(session_metadata)
                }
            ]
        )
        
        print(f"Stored session metadata for {session_id}: {row_count} rows at {s3_parquet_path}")
        return True
...
def validate_required_columns(df: pd.DataFrame, required_cols: List[str] = None) -> bool:
...
def standardize_amount_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for amount_field in FIELD_MAPPINGS['AMOUNT_FIELDS']:
        if amount_field in df_copy.columns:
            df_copy[amount_field] = pd.to_numeric(df_copy[amount_field], errors='coerce').fillna(0)
    return df_copy


def get_claim_text_fields(claim: pd.Series) -> str:
    """Extract and combine all text fields from a claim for analysis."""
    text_fields = ['note_text', 'lossdescription', 'injurydescription', 'claim_notes', 'description']
    text_parts = []
