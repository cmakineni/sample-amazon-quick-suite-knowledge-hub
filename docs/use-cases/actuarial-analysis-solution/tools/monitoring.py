...
class MonitoringService:
    def __init__(self, monitoring_config=None):
        ...
        self.kpi_targets = self.config.get('kpi_targets', {
            'loss_ratio': 0.65,
    ...
    def monitor_development(self, claims_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            df = pd.DataFrame(claims_data['data'] if 'data' in claims_data else claims_data)
    ...
    def _calculate_kpis(self, df: pd.DataFrame) -> List[KPI]:
        ...
        try:
            ...
            if 'totalincurred' in df.columns and 'paidtotal' in df.columns:
                ...
                total_reserves = df['reservetotal'].sum() if 'reservetotal' in df.columns else 0
                ...
                kpis.append(KPI(
                    ...
                    threshold_lower=0.4,
                    status='above_threshold' if loss_ratio > self.alert_thresholds['loss_ratio'] else 'normal',
                    trend='stable'
                ))
                ...
                # Reserve Ratio KPI
                reserve_ratio = total_reserves / total_incurred if total_incurred > 0 else 0
                kpis.append(KPI(
                    name='reserve_ratio',
                    ...
                    status='above_threshold' if reserve_ratio > 0.50 else 'below_threshold' if reserve_ratio < 0.10 else 'normal',
                    trend='stable'
                ))
                ...
                kpis.append(KPI(
                    ...
                    status='above_threshold' if avg_severity > self.kpi_targets['avg_severity'] * 1.5 else 'normal',
                    trend='stable'
                ))
            ...
            if len(df) > 0:
                ...
                kpis.append(KPI(
                    ...
                    status='above_threshold' if claim_count > 200 else 'below_threshold' if claim_count < 50 else 'normal',
                    trend='stable'
                ))
                ...
                if 'totalincurred' in df.columns:
                    ...
                    kpis.append(KPI(
                        ...
                        threshold_lower=1.0,
                        status='above_threshold' if large_claims_pct > 15.0 else 'normal',
                        trend='stable'
                    ))
                ...
                if 'claimstatus' in df.columns:
                    ...
                    kpis.append(KPI(
                        ...
                        status='above_threshold' if open_claims_ratio > 60.0 else 'normal',
                        trend='stable'
                    ))
            
            # Time-based KPIs
            if 'policyeffectivedate' in df.columns and 'note_date' in df.columns:
                df['policy_date'] = pd.to_datetime(df['policyeffectivedate'], errors='coerce')
                ...
                if len(valid_dates) > 0:
                    reporting_delays = (valid_dates['report_date'] - valid_dates['policy_date']).dt.days
                    ...
                    kpis.append(KPI(
                        ...
                        threshold_lower=5.0,
                        status='above_threshold' if avg_reporting_delay > 90.0 else 'normal',
                        trend='stable'
                    ))
            ...
            for kpi in kpis:
                if kpi.status == 'above_threshold':
                    alerts.append(Alert(
                        ...
                        data_context={'kpi_name': kpi.name, 'current': float(kpi.current_value), 'threshold': float(kpi.threshold_upper)}
                    ))
        except Exception as e:
    ...
    def _generate_dashboard_metrics(self, df: pd.DataFrame, kpis: List[KPI]) -> Dict[str, Any]:
        metrics = {
            'summary_statistics': {
                'total_claims': len(df),
                'total_incurred': float(df['totalincurred'].sum()) if 'totalincurred' in df.columns else 0,
                'total_paid': float(df['paidtotal'].sum()) if 'paidtotal' in df.columns else 0,
                'total_reserves': float(df['reservetotal'].sum()) if 'reservetotal' in df.columns else 0,
                'avg_claim_size': float(df['totalincurred'].mean()) if 'totalincurred' in df.columns else 0,
                'median_claim_size': float(df['totalincurred'].median()) if 'totalincurred' in df.columns else 0,
                'max_claim_size': float(df['totalincurred'].max()) if 'totalincurred' in df.columns else 0
            },
            ...
            'performance_indicators': {
                'claims_per_day': len(df) / 30 if len(df) > 0 else 0,  # Assuming 30-day period
                'avg_reserve_per_claim': float(df['reservetotal'].mean()) if 'reservetotal' in df.columns else 0,
    ...
    def _analyze_claim_distribution(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'medium_claims_10k_50k': len(amounts[(amounts > 10000) & (amounts <= 50000)]),
            'large_claims_50k_100k': len(amounts[(amounts > 50000) & (amounts <= 100000)]),
    ...
    def _analyze_by_line_of_business(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for lob in df['lineofbusiness'].unique():
            ...
            lob_analysis[str(lob)] = {
                ...
                'total_incurred': float(lob_data['totalincurred'].sum()) if 'totalincurred' in lob_data.columns else 0,
                'avg_severity': float(lob_data['totalincurred'].mean()) if 'totalincurred' in lob_data.columns else 0,
        ...
        try:
            ...
            for year in df['accident_year'].dropna().unique():
                ...
                yearly_analysis[str(int(year))] = {
                    ...
                    'total_incurred': float(year_data['totalincurred'].sum()) if 'totalincurred' in year_data.columns else 0,
                    'avg_severity': float(year_data['totalincurred'].mean()) if 'totalincurred' in year_data.columns else 0
                }
            ...
            return {
                ...
                'trend_analysis': {
                    ...
                    'most_active_year': max(yearly_analysis.keys(), key=lambda x: yearly_analysis[x]['claim_count']) if yearly_analysis else None
                }
            }
        except Exception as e:
    ...
    def _analyze_claim_status(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'open_vs_closed': {
                ...
                'closed_claims': int(status_counts.get('Close', 0) + status_counts.get('Closed', 0)),
                'open_percentage': float((status_counts.get('Open', 0) / total_claims) * 100)
            }
        }
    ...
    def _calculate_settlement_rate(self, df: pd.DataFrame) -> float:
        ...
        closed_claims = sum(df['claimstatus'].str.contains('|'.join(closed_statuses), case=False, na=False))
