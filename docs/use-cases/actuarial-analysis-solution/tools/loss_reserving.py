...
class LossReservingService:
    ...
    def build_loss_triangles(self, claims_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing_cols:
                col_mapping = {
                    'note_date': ['lossdate', 'loss_date', 'date_of_loss', 'accident_dt', 'accident_date'],
                    'report_date': ['reportdate', 'report_dt', 'date_reported', 'reported_date']
                }
                ...
                for req_col in missing_cols:
                    ...
                    if not found:
                        return {'error': f'Required column {req_col} not found. Available columns: {list(df.columns)}'}
            ...
            df['development_months'] = ((df['report_date'] - df['accident_date']).dt.days / 30.44).round(0).astype(int)
            df['development_years'] = (df['development_months'] / 12).round(0).astype(int) + 1
            
            # Ensure numeric columns
            df['totalincurred'] = pd.to_numeric(df['totalincurred'], errors='coerce').fillna(0)
            df['paidtotal'] = pd.to_numeric(df['paidtotal'], errors='coerce').fillna(0)
            df['reservetotal'] = pd.to_numeric(df['reservetotal'], errors='coerce').fillna(0)
            df['accident_year'] = pd.to_numeric(df['accident_year'], errors='coerce')
            df['development_years'] = pd.to_numeric(df['development_years'], errors='coerce')
            
            # Remove invalid years
            df = df.dropna(subset=['accident_year', 'development_years'])
            
            # Aggregate by accident year and development years
            triangle_data = df.groupby(['accident_year', 'development_years']).agg({
                'totalincurred': 'sum',
                'paidtotal': 'sum',
                'reservetotal': 'sum',
                'claimnumber': 'count'
        ...
        try:
            ...
            for accident_year in incurred_cumulative.index:
                ...
                if latest_value > 0 and latest_period is not None:
                    ...
                    tail_factor = 1.10 if current_period_idx < len(sorted_columns) - 2 else 1.05
            ...
            return {
                'development_factors': {k: float(v) for k, v in development_factors.items()},
                ...
                'summary': {
                    ...
                    'overall_development_factor': float(total_ultimate / total_current) if total_current > 0 else 1.0,
                    'ibnr_percentage': int((total_ibnr / total_current * 100)) if total_current > 0 else 0
                }
            }
    ...
    def calculate_bornhuetter_ferguson(self, triangles_data, chain_ladder_result):
        ...
        try:
            ...
            for year in incurred_cumulative.index:
                ...
                estimated_policies = max(50, current_incurred / (avg_claim_size * claim_frequency))
            ...
            for year_str in total_incurred_by_year:
                if estimated_policies_by_year[year_str] > 0:
                    loss_per_policy_by_year[year_str] = total_incurred_by_year[year_str] / estimated_policies_by_year[year_str]
            
            if loss_per_policy_by_year:
                recent_years = sorted(loss_per_policy_by_year.keys())[-3:]
                expected_loss_per_policy = sum(loss_per_policy_by_year[y] for y in recent_years) / len(recent_years)
            else:
            ...
            for year in incurred_cumulative.index:
                ...
                percent_developed = min(0.95, 1.0 / cumulative_dev_factor) if cumulative_dev_factor > 1 else 0.80
                
                # BF Formula: Ultimate = Paid + (Expected Ultimate - Paid) × (1 - % Developed)
                bf_ultimate = current_paid + (expected_ultimate - current_paid) * (1 - percent_developed)
                bf_ultimate = max(current_incurred * 1.02, bf_ultimate)  # At least 2% above current
            ...
            return {
                ...
                'expected_loss_ratios': {str(k): float(v) for k, v in loss_per_policy_by_year.items()},
                'assumptions': {
    ...
    def calculate_confidence_intervals(self, triangles_data, n_simulations=1000):
        ...
        try:
            ...
            return {
                ...
                'std_dev': (sum((x - sum(base_reserves)/n)**2 for x in base_reserves) / n) ** 0.5,
    ...
    def test_reserve_adequacy(self, chain_ladder_result, bf_result):
        ...
        try:
            ...
            methodology_difference = abs(cl_reserves - bf_reserves) / max(cl_reserves, bf_reserves, 1)
            industry_benchmark = max(cl_reserves, bf_reserves) * 0.1
            
            adequacy_ratio = min(cl_reserves, bf_reserves) / max(cl_reserves, bf_reserves, 1)
            ...
            return {
                ...
                'adequacy_tests': {
                    ...
                    'benchmark_comparison': bool(max(cl_reserves, bf_reserves) >= industry_benchmark),
                    'overall_adequate': bool(adequacy_ratio > 0.8 and methodology_difference < 0.2)
                }
            }
...
def calculate_reserves(triangles_data):
    ...
    try:
        ...
        bf_result = service.calculate_bornhuetter_ferguson(triangles_data, chain_ladder_result)
        ...
        return {
            ...
            'methodology_comparison': service.compare_methodologies(chain_ladder_result, bf_result),
            'summary': {
                'total_ibnr_chain_ladder': float(chain_ladder_result.get('summary', {}).get('total_ibnr', 0)),
                'total_ibnr_bf': float(bf_result.get('total_ibnr', 0)),
                'confidence_75_pct': float(confidence_intervals.get('percentile_75', 0)),
                'confidence_90_pct': float(confidence_intervals.get('percentile_90', 0)),
                'confidence_95_pct': float(confidence_intervals.get('percentile_95', 0)),
                'reserve_adequacy_ratio': float(adequacy_test.get('adequacy_ratio', 0)),
                'recommended_reserves': float(max(
