...
class AgentCoreGatewayStack(NestedStack):
    def __init__(self, scope: Construct, construct_id: str, 
        ...
        gateway_creator_role.add_to_policy(iam.PolicyStatement(
            actions=[
                "bedrock-agentcore:*",
                "cognito-idp:*", 
                "iam:CreateRole",
    ...
    def _get_gateway_lambda_code(self):
        ...
        # Load KB tools
        kb_tools_path = os.path.join(os.path.dirname(__file__), '..', 'tools', 'kb_agentcore_tools.json')
        with open(kb_tools_path, 'r', encoding="utf-8") as f:
            kb_tools_json = json.dumps(json.load(f))
        
        # Get base lambda code and inject JSON
        lambda_code = self._get_base_gateway_lambda_code()
        lambda_code = lambda_code.replace('KB_TOOLS_PLACEHOLDER', kb_tools_json)
...
def handler(event, context):
    ...
    try:
        ...
        if event['RequestType'] == 'Create':
            ...
            import random
            gateway_name = "bedrock-kb-quicksuite-gateway"
            ...
            try:
                kb_tools_json = """KB_TOOLS_PLACEHOLDER"""
                kb_tools = json.loads(kb_tools_json)
            ...
            try:
                client.create_mcp_gateway_target(
                    gateway=gateway, name="kb-tools-target", target_type="lambda",
                    target_payload={"lambdaArn": os.environ['KB_LAMBDA_ARN'], 
                                  "toolSchema": {"inlinePayload": kb_tools}}
                )
            except Exception as e:
