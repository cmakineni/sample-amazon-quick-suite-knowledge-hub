...
def query_knowledge_bases(bedrock_runtime, parameters: Dict[str, Any]) -> Dict[str, Any]:
    """Query a Knowledge Base with natural language."""
    
    try:
        # Extract parameters
        query = parameters.get('query') or parameters.get('text')
        knowledge_base_id = (parameters.get('knowledge_base_id') or 
                           parameters.get('knowledgeBaseId') or 
                           parameters.get('kb_id'))
        ...
        if not query or not knowledge_base_id:
            return {
                'statusCode': 400,
                'body': json.dumps({'error': 'query and knowledge_base_id are required'})
            }
        ...
        for result in response.get('retrievalResults', []):
            ...
            location_uri = location.get('uri', '') if isinstance(location, dict) else str(location)
